...
const Likewatchlatersavebtns = ({vv,vid}) => {
  ...
  useEffect(()=>{
    if (currentuser?.result?._id) {
      const likedVideos = Array.isArray(likedvideolist?.data) 
        ? likedvideolist.data 
        : [];
      ...
      const watchLaterVideos = Array.isArray(watchlaterlist?.data) 
        ? watchlaterlist.data 
        : [];
...
const togglelikevideo = (e, lk) => {
  ...
  // Ensure lk is a number and default to 0 if undefined
  const currentLikes = Number(lk) || 0;

  if (likebtn) {
    // Unlike the video
    setlikebtn(false);
    dispatch(likevideo({ id: vid, Like: -1 })); // Decrement
    dispatch(deletelikedvideo({ videoid: vid, viewer: currentuser?.result?._id }));
  } else {
    // Like the video
    setlikebtn(true);
    dispatch(likevideo({ id: vid, Like: 1 })); // Increment
